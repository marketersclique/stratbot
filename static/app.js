...
async function redirectToSignupPage() {
  ...
  // IMPORTANT: Redirect to a special page that checks authentication and redirects accordingly
  // This avoids CORS issues and ensures we can check cookies on the main app domain
  // The check-and-redirect page will:
  // - If authenticated: Redirect back with token
  // - If not authenticated: Redirect to signup/login
  const checkAuthUrl = `${mainAppUrl}/auth/check-and-redirect?returnUrl=${encodeURIComponent(cleanUrl)}&action=calendar`;
  window.location.href = checkAuthUrl; // Same tab navigation
}
...
async function checkBackendAuthentication() {
  try {
    ...
    // Method 4: If no token found locally, try to get session from main app (cross-origin)
    // This handles the case where user is authenticated on marketersclique.com but cookies aren't accessible
    if (!authToken) {
      console.log('🔄 No token found locally, checking if user is authenticated on main app...');
      try {
        const mainAppUrl = getMainAppUrl();
        // Try to get session info from main app via a special endpoint or iframe
        // Note: This requires CORS to be enabled on the main app
        const sessionCheckUrl = `${mainAppUrl}/api/auth/check-session`;

        // Try to fetch session from main app (this will only work if CORS allows it)
        const sessionResponse = await fetch(sessionCheckUrl, {
          method: 'GET',
          credentials: 'include', // Include cookies
          mode: 'cors'
        }).catch(err => {
          console.log('Could not check session from main app (CORS or endpoint not available):', err);
          return null;
        });
        ...
        if (sessionResponse && sessionResponse.ok) {
          ...
          if (sessionData && sessionData.token) {
            ...
            tokenSource = 'main-app-api';
            console.log('✅ Found token from main app API');
          }
        }
      } catch (crossOriginError) {
        console.log('Cross-origin session check failed (expected if CORS not enabled):', crossOriginError);
      }
    }
    ...
    if (!authToken) {
      ...
      try {
        const mainAppUrl = getMainAppUrl();
        // Try to get session info from main app via a special endpoint
        // This endpoint checks if user is authenticated and returns their token
        const sessionCheckUrl = `${mainAppUrl}/api/auth/check-session`;

        // Try to fetch session from main app (this will only work if CORS allows it)
        const sessionResponse = await fetch(sessionCheckUrl, {
          method: 'GET',
          credentials: 'include', // Include cookies from main app
          mode: 'cors',
          headers: {
        ...
        if (sessionResponse && sessionResponse.ok) {
          ...
          if (sessionData && sessionData.authenticated && sessionData.token) {
            authToken = sessionData.token;
            tokenSource = 'main-app-api';
            console.log('✅ Found token from main app API - user is authenticated on main app');
          } else {
            console.log('User is not authenticated on main app');
          }
        }
      } catch (crossOriginError) {
        console.log('Cross-origin session check failed (expected if CORS not enabled):', crossOriginError);
    ...
    if (!authToken) {
      ...
      console.log('💡 Suggestion: User should click "Generate Calendar" to be redirected to signup/login');
...
function checkPendingCalendarGeneration() {
  ...
  // Check URL parameters for action=calendar and token
  const urlParams = new URLSearchParams(window.location.search);
  const action = urlParams.get('action');
  const tokenInUrl = urlParams.get('token'); // Check if token is in URL (from login/signup redirect)

  // Also check localStorage for pending calendar flag and stored token
  const pendingCalendar = localStorage.getItem('stratbot_pending_calendar');
  const storedToken = localStorage.getItem('stratbot_auth_token');

  // IMPORTANT: If token is in URL OR stored in localStorage (user just logged in/signed up),
  // always update auth status to ensure header is updated
  // This ensures header is updated even if there's no pending calendar
  const hasToken = tokenInUrl || (storedToken && storedToken.length > 50);

  if (hasToken) {
    console.log('Token found (URL or localStorage) - user returned from login/signup, updating auth status...');
    // Extract token from URL if present (this stores it in localStorage)
    if (tokenInUrl) {
      extractTokenFromUrl();
    }
    // Update auth status to refresh header (with small delay to ensure token is stored)
    setTimeout(() => {
      updateAuthStatus().then(() => {
        console.log('Auth status updated after login/signup');
      }).catch(error => {
        console.error('Error updating auth status:', error);
      });
    }, 100);
  ...
  if (action === 'calendar' || pendingCalendar === 'true') {
    ...
    // No pending calendar, but check if user just logged in (token in URL or localStorage)
    // This handles the case where user logs in but doesn't have pending calendar flag
    const urlParams = new URLSearchParams(window.location.search);
    const tokenInUrl = urlParams.get('token');
    const storedToken = localStorage.getItem('stratbot_auth_token');
    const hasToken = tokenInUrl || (storedToken && storedToken.length > 50);

    // If token is present, user just returned from login/signup - update auth status
    if (hasToken) {
      console.log('No pending calendar, but token found - user returned from login/signup');
      ...
      setTimeout(() => {
        updateAuthStatus().then(() => {
          console.log('Auth status updated after login/signup (no pending calendar)');
        }).catch(error => {
          console.error('Error updating auth status:', error);
        });
      }, 100);
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // If token was extracted, wait a bit for it to be stored, then update
  if (extractedToken) {
    // Token was extracted - update auth status after a short delay to ensure it's stored
    setTimeout(() => {
      updateAuthStatus();
    }, 100);
  } else {
    // No token in URL - update immediately
    updateAuthStatus();
  }
